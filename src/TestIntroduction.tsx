...
const TestIntroduction: React.FC = () => {
  ...
  useEffect(() => {
    setLoading(true);
    if (testId && studentId) {
      axios.get(`https://staging-exskilence-be.azurewebsites.net/api/student/test/instuction/${studentId}/${testId}/`)
        .then(response => {
          setLoading(false);
...
//     // Optimistic update: navigate to the test section immediately
//     navigate("/test-section", { state: { testId } });

//     try {
//       // Attempt to sync with the server
//       await axios.get(`https://staging-exskilence-be.azurewebsites.net/api/student/test/instuction/${studentId}/${testId}/`);
//     } catch (error) {
