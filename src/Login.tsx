                ...
                const fetchData = async () => {
                  try {
                    const response = await axios.get(
                      `https://staging-exskilence-be.azurewebsites.net/api/login/${email}/`
                    );
                    
                    const enctyptedStudentId = CryptoJS.AES.encrypt(response.data.student_id, secretKey).toString();
                    const enctyptedCourseId = CryptoJS.AES.encrypt(response.data.course_id, secretKey).toString();
                    const enctyptedbatchId = CryptoJS.AES.encrypt(response.data.batch_id, secretKey).toString();
                    sessionStorage.setItem('StudentId', enctyptedStudentId);
                    sessionStorage.setItem('CourseId', enctyptedCourseId);
                    sessionStorage.setItem("BatchId", enctyptedbatchId);
